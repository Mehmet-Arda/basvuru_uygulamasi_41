...
class DgsApplicationScreen extends StatefulWidget{

  late Map studentInfo;

  DgsApplicationScreen(studentInfo){
    this.studentInfo=studentInfo;
  }

  @override
  _DgsApplicationScreenState createState()=>_DgsApplicationScreenState(studentInfo);


}

class _DgsApplicationScreenState extends State<DgsApplicationScreen> {
  late Map studentInfo;
  ...
  List <String> downloadDestination=[
    "applications_files/default_files/default_dgs_files/default_dgs_pdfs/Genel_amacli_Dilekce_Ornegi.pdf",
    "applications_files/default_files/default_dgs_files/default_dgs_docs/Dgs_document.docx"
  ];

  _DgsApplicationScreenState(studentInfo) {
    this.studentInfo = studentInfo;
  }
            ...
            Column(
              ...
              children: [
                ...
                ElevatedButton(onPressed:() {
                  StorageHelper.downloadFile(downloadDestination[0],"dgs.pdf");
                  StorageHelper.downloadFile(downloadDestination[1],"dgs.docx").then((value) {
  ...
  Future selectFile()async{
    ...
    else{setState(() {isFileSelected=true;fileNameforText="1.dosya: ${result.files[0].path!.split("/").last}\n2.dosya: ${result.files[1].path!.split("/").last}";});}
  ...
  Future addApplication()async{
    ...
    final destination0 = "applications_files/dgs_applications_files/dgs_applications_pdfs/$fileName0";

    final fileName1="${studentInfo["schoolNumber"]}_${studentInfo["studentName"]}_${studentInfo["studentSurname"]}_$now.${extens1}";
    final destination1 = "applications_files/dgs_applications_files/dgs_applications_docs/$fileName1";

    List <String> uploadDestination=[destination0,destination1];

    StudentHelper _studentHelper=StudentHelper();

    _studentHelper.addDgsApplication(uploadDestination,file,"Dgs",studentInfo,schoolName,facultyName,departmentName);
